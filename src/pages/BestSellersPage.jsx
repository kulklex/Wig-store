...
const BestSellersPage = () => {
  ...
  const categories = [...new Set(bestSellers.map((product) => product.category))];
  ...
  if (bestSellersLoading && bestSellers.length === 0) {
    return (
      <div className="container py-5 text-center">
        ...
        <p className="mt-3 text-muted">Loading best sellers...</p>
      </div>
    );
  }
    ...
    return (
      <div className="container py-5">
        <div className="alert alert-danger" role="alert">
          <h4>Error Loading Best Sellers</h4>
          <p className="mb-0">{bestSellersError}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="container-fluid py-4">
        ...
        <div className="gap-3">
          {hasActiveFilters && (
            <span className="badge bg-warning text-dark fs-6 px-3 py-2">
              {filteredProducts.length} of {bestSellers.length} shown
            </span>
          )}
        </div>
      ...
      <main className='row gx-3 md:justify-content-center'>
